...
inat_csv = r"W:\Heritage\Heritage_Data\CommSciVet\iNatDataTest_first.csv"
inat_fields = ["id","observed_on","user_id","user_login","created_at","updated_at","license","url","image_url","sound_url","tag_list","description","captive_cultivated","place_guess","latitude","longitude","positional_accuracy","private_place_guess","private_latitude","private_longitude","public_positional_accuracy","geoprivacy","taxon_geoprivacy","coordinates_obscured","positioning_method","positioning_device","place_county_name","scientific_name","common_name","iconic_taxon_name","taxon_id","taxon_order_name","taxon_family_name","field_count","SHAPE@"]
...
inat_sdf.rename(columns={'field:count':'field_count'}, inplace=True) # rename field:count to deal with bad character
...
temp_inat_import = inat_sdf.spatial.to_featureclass(location=os.path.join(os.path.dirname(comm_fc),"temp_inat_import"))

# project into custom albers using existing CommSciVet layer
sref = arcpy.Describe(comm_fc).spatialReference
temp_inat_import = arcpy.Project_management(temp_inat_import,os.path.join(os.path.dirname(comm_fc),"temp_inat_import_albers"),sref)
...
if len(added_ids) == 0:
    ...
    added_dict = {int(row[0]): row[0:] for row in arcpy.da.SearchCursor(temp_inat_import,inat_fields) if row[0] in added_ids}
